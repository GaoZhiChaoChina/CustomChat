...
- (void)createTableView {
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 20, kScreenWidth, kScreenHeight - 44 - 20)
...
- (void)createBottomView {
    ChatInputView *inputView = [[ChatInputView alloc] initWithFrame:CGRectMake(0, kScreenHeight - 44, kScreenWidth, 44)];
...
- (void)inputView:(ChatInputView *)inputView willShowKeyboardHeight:(CGFloat)height time:(NSNumber *)time {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.view.frame = CGRectMake(0, -height, kScreenWidth, kScreenHeight);
    }];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if ([self.inputView isEditing] || scrollView.contentOffset.y - (scrollView.contentSize.height - kScreenHeight) <= 200) return;
    //    NSLog(@"%lf---%lf", scrollView.contentOffset.y, scrollView.contentSize.height - kScreenHeight);
